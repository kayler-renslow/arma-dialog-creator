...
/**
 A {@link ControlClassOld} is merely a header file (.h) class that is:<br>
 <ol>
 <li>Extend-able via {@link ControlClassOld#extendControlClass(ControlClassOld)}</li>
...
public class ControlClassOld {
	...
	private final List<ControlClassOld> requiredNestedClasses = new LinkedList<>();
	private final List<ControlClassOld> optionalNestedClasses = new LinkedList<>();
	private final List<ControlClassOld> inheritedNestedClasses = new LinkedList<>();
	private final ReadOnlyList<ControlClassOld> requiredNestedClassesReadOnly = new ReadOnlyList<>(requiredNestedClasses);
	private final ReadOnlyList<ControlClassOld> optionalNestedClassesReadOnly = new ReadOnlyList<>(optionalNestedClasses);
	private final ReadOnlyList<ControlClassOld> inheritedNestedClassesReadOnly = new ReadOnlyList<>(inheritedNestedClasses);
	private final List<ControlClassOld> tempNestedClasses = new LinkedList<>();
	private final ReadOnlyList<ControlClassOld> tempNestedClassesReadOnly = new ReadOnlyList<>(tempNestedClasses);
	...
	private final ValueObserver<ControlClassOld> extendClassObserver = new ValueObserver<>(null);
	private final Set<ControlClassOld> mySubClasses = new HashSet<>();

	/** set to true when {@link #extendControlClass(ControlClassOld)} is invoked, false after the method has completed */
	...
	private final UpdateGroupListener<ControlClassUpdate> controlClassUpdateExtendListener = new UpdateGroupListener<ControlClassUpdate>() {
		...
		private void handleTempNestedClassUpdate(ControlClassTemporaryNestedClassUpdate update) {
			ControlClassOld mine = findNestedClassNullable(update.getNestedClass().getClassName());
			ControlClassOld updateNested = update.getNestedClass();
			if (update.isAdded()) {
				if (mine == null) {
			...
			controlClassUpdateGroup.update(
					new ControlClassTemporaryNestedClassUpdate(ControlClassOld.this, updateNested, update.isAdded())
			);
		}
	...
	private final UpdateGroupListener<ControlPropertyUpdate> controlPropertyListener = new UpdateGroupListener<ControlPropertyUpdate>() {
		...
		public void update(@NotNull UpdateListenerGroup<ControlPropertyUpdate> group, ControlPropertyUpdate data) {
			if (data instanceof ControlPropertyInheritUpdate) {
				...
				controlClassUpdateGroup.update(
						new ControlClassInheritPropertyUpdate(ControlClassOld.this,
								update.getControlProperty(), update.wasInherited(), updatingExtendClass
						)
				);
			}
			propertyUpdateGroup.update(data);
			controlClassUpdateGroup.update(new ControlClassPropertyUpdate(ControlClassOld.this, data));
		}
	};
	...
	/**
	 ...
	 <p>
	 This constructor will invoke {@link #ControlClassOld(String, ControlClassRequirementSpecification, SpecificationRegistry, DefaultValueProvider.Context)},
	 ...
	 @see ControlProperty
	 */
	public ControlClassOld(@NotNull String name, @NotNull ControlClassRequirementSpecification specification, @NotNull SpecificationRegistry registry) {
		this(name, specification, registry, new DefaultValueProvider.ControlClassNameContext(name));
	}


	/**
	 This is equivalent to {@link #ControlClassOld(String, ControlClassRequirementSpecification, SpecificationRegistry)},
	 but this constructor allows for a predefined {@link DefaultValueProvider.Context}.
	 <p>
	 All nested classes for this {@link ControlClassOld} will then get a new {@link DefaultValueProvider.ControlClassNameContext}
	 ...
	 */
	public ControlClassOld(@NotNull String name, @NotNull ControlClassRequirementSpecification specification,
	...
	/**
	 Construct a {@link ControlClassOld} with the given specification and {@link SpecificationRegistry}.
	 ...
	 */
	public ControlClassOld(@NotNull ControlClassSpecification specification, @NotNull SpecificationRegistry registry) {
	...
	public ControlClassOld(@NotNull ControlClassSpecification specification, @NotNull SpecificationRegistry registry,
		...
		if (specification.getExtendClassName() != null) {
			ControlClassOld extendMe = registry.findControlClassByName(specification.getExtendClassName());
			if (extendMe == null) {
				throw new IllegalArgumentException("couldn't find ControlClassOld " + specification.getExtendClassName() + " to extend for " + this.getClassName());
			}
	...
	private void afterPropertyAndNestedClassConstruction() {
		...
		for (ControlClassOld nestedClass : getAllNestedClasses()) {
		...
		classNameObserver.addListener(new ValueListener<String>() {
			...
			public void valueUpdated(@NotNull ValueObserver<String> observer, String oldValue, String newValue) {
				...
				controlClassUpdateGroup.update(new ControlClassRenameUpdate(ControlClassOld.this, oldValue, newValue));
			}
		});
	}
	...
	private void addNestedClasses(@NotNull List<ControlClassOld> nestedClasses, @NotNull SpecificationRegistry registry,
								  @NotNull List<ControlClassSpecification> nestedClassesSpecs,
								  @Nullable DefaultValueProvider.Context context) {
		for (ControlClassSpecification nestedClass : nestedClassesSpecs) {
			nestedClasses.add(new ControlClassOld(nestedClass, registry, new DefaultValueProvider.ControlClassNameContext(context, nestedClass.getClassName())));
		}
	}
	...
	/**
	 Check if the given class will create an inheritance loop if this ControlClassOld were to extend it.
	 ...
	 @see #extendControlClass(ControlClassOld)
	 */
	public boolean hasInheritanceLoop(@NotNull ControlClassOld other) {
		...
		LinkedList<ControlClassOld> toVisit = new LinkedList<>();
		HashSet<String> visited = new HashSet<>();
		toVisit.add(this.getExtendClass());
		while (!toVisit.isEmpty()) {
			ControlClassOld visit = toVisit.pop();
			if (visit.getClassName().equals(other.getClassName())) {
				return true;
			}
			visited.add(visit.getClassName());

			ControlClassOld extendClass = visit.getExtendClass();
	...
	/**
	 Extend the given {@link ControlClassOld}, or clear the extend class with null.
	 <p>
	 For each property in this {@link ControlClassOld}: if it initially has a value in it
	 ...
	 <p>
	 Any of {@link ControlClassOld#getAllNestedClasses()} that don't exist in this class will be inherited.
	 ...
	 @throws IllegalArgumentException if there is an inheritance loop ({@link #hasInheritanceLoop(ControlClassOld)})
	 @see ControlProperty#inherit(ControlProperty)
	 */
	public final void extendControlClass(@Nullable ControlClassOld extendMe) {
		ControlClassOld oldExtendClass = getExtendClass();
		...
		if (extendMe != null) {
			...
			//attempt to inherit all properties, if they aren't defined in this ControlClassOld
			...
			for (ControlClassOld nested : extendMe.getAllNestedClasses()) {
				ControlClassOld nestedMatch = findNestedClassNullable(nested.getClassName());
				if (nestedMatch == null) {
			...
			for (ControlClassOld tempNested : tempNestedClasses) {
			...
			for (ControlClassOld subClass : mySubClasses) {
				...
				for (ControlClassOld tempNested : subClass.tempNestedClasses) {
					boolean matched = false;
					for (ControlClassOld myNestedClass : getAllNestedClasses()) {
	...
	public final boolean inheritNestedClass(@NotNull String nestedClassName) {
		...
		ControlClassOld parentNestedClass = getExtendClass().findNestedClassNullable(nestedClassName);
		if (parentNestedClass == null) {
			return false;
		}

		ControlClassOld mine = null;
		for (ControlClassOld inherited : getAllNestedClasses()) {
		...
		final ControlClassOld finalMine = mine;
	...
	public final void overrideNestedClass(@NotNull String nestedClassName) {
		...
		ControlClassOld mine = null;
		for (ControlClassOld inherited : inheritedNestedClasses) {
			if (inherited.getClassName().equals(nestedClassName)) {
				mine = inherited;
				break;
			}
	...
	/**
	 @return the {@link ControlClassOld} that this instance is extending, or null if extending nothing
	 @see #extendControlClass(ControlClassOld)
	 */
	@Nullable
	public final ControlClassOld getExtendClass() {
		return extendClassObserver.getValue();
	}

	/** @return a {@link ReadOnlyValueObserver} for {@link #getExtendClass()} */
	public final ReadOnlyValueObserver<ControlClassOld> getExtendClassReadOnlyObserver() {
	...
	@NotNull
	public final ReadOnlyList<ControlClassOld> getRequiredNestedClasses() {
		return requiredNestedClassesReadOnly;
	}

	@NotNull
	public final ReadOnlyList<ControlClassOld> getOptionalNestedClasses() {
	 ...
	 */
	@NotNull
	public final Iterable<ControlClassOld> getAllNestedClasses() {
		ArrayList<List<ControlClassOld>> merge = new ArrayList<>(2);
		merge.add(requiredNestedClassesReadOnly);
		merge.add(optionalNestedClassesReadOnly);
		return new ListsIterator<>(merge);
	}

	@NotNull
	public final Iterable<ControlClassOld> getAllInheritedNestedClasses() {
	...
	/**
	 ...
	 The search will be done inside the {@link ControlClassOld#getRequiredProperties()} return value.
	...
	/**
	 Get the control property instance for the given lookup item.
	 The search will be done inside the {@link ControlClassOld#getOptionalProperties()} return value.
	...
	/**
	 ...
	 @return the ControlProperty instance
	 @throws IllegalArgumentException when the lookup doesn't exist in the {@link ControlClassOld}
	...
	/**
	 Find a required nested {@link ControlClassOld} by name. If it can't be found, will throw an exception.

	 @return matched class
	 @see #findRequiredNestedClassNullable(String)
	 */
	@NotNull
	public final ControlClassOld findRequiredNestedClass(@NotNull String className) {
		ControlClassOld c = findRequiredNestedClassNullable(className);
		if (c != null) {
			return c;
		}
	...
	/**
	 Find a required nested {@link ControlClassOld} by name. If it can't be found, will return null

	 @return matched class, or null
	 @see #findRequiredNestedClass(String)
	 */
	@Nullable
	public final ControlClassOld findRequiredNestedClassNullable(@NotNull String className) {
		return findNestedClassFromList(className, requiredNestedClasses);
	}

	/**
	 Find an optional nested {@link ControlClassOld} by name. If it can't be found, will throw an exception.

	 @return matched class
	 @see #findOptionalNestedClassNullable(String)
	 */
	@NotNull
	public final ControlClassOld findOptionalNestedClass(@NotNull String className) {
		ControlClassOld c = findOptionalNestedClassNullable(className);
		if (c != null) {
			return c;
		}
	...
	/**
	 Find an optional nested {@link ControlClassOld} by name. If it can't be found, will return null

	 @return matched class, or null
	 @see #findOptionalNestedClass(String)
	 */
	@Nullable
	public final ControlClassOld findOptionalNestedClassNullable(@NotNull String className) {
		return findNestedClassFromList(className, optionalNestedClasses);
	}

	/**
	 Find an optional or required nested {@link ControlClassOld} by name. If it can't be found, will throw an exception

	 @return matched class
	 @see #findNestedClassNullable(String)
	 */
	@NotNull
	public final ControlClassOld findNestedClass(@NotNull String className) {
		ControlClassOld c = findNestedClassNullable(className);
		if (c != null) {
			return c;
		}
	...
	/**
	 Find an optional or required nested {@link ControlClassOld} by name. If it can't be found, will return null

	 @return matched class, or null
	 @see #findNestedClass(String)
	 */
	@Nullable
	public final ControlClassOld findNestedClassNullable(@NotNull String className) {
		ControlClassOld c = findRequiredNestedClassNullable(className);
		if (c != null) {
			return c;
		}
	...
	@Nullable
	private ControlClassOld findNestedClassFromList(@NotNull String className, @NotNull Iterable<ControlClassOld> list) {
		for (ControlClassOld controlClass : list) {
	...
	/**
	 ...
	 doesn't have a value, it will be automatically inherited by {@link #extendControlClass(ControlClassOld)} invocations
	 or any other inheritance related updates.
	 <p>
	 If the given property is a temporary property (doesn't actually exist in the {@link ControlClassOld},
	 but does in {@link #extendControlClass(ControlClassOld)} and was inherited), the temporary property
	 ...
	 @throws IllegalArgumentException when the property doesn't exist in this {@link ControlClassOld}
	...
	/**
	 ...
	 <li>If the lookup isn't found in this {@link ControlClassOld} and {@link #getExtendClass()} is null, nothing will happen.</li>
	 <li>
	 If the lookup isn't found in this {@link ControlClassOld} but is inside {@link #getExtendClass()},
	 ...
	 <li>
	 If the lookup is found in this {@link ControlClassOld} but isn't inside {@link #getExtendClass()},
	 nothing will happen.
	 </li>
	 </ul>
	 If at any point, one of the above conditions changes because {@link #extendControlClass(ControlClassOld)} is
	 invoked, the properties should react accordingly due to {@link ControlClassOld#getControlClassUpdateGroup()}
	 listeners.
	...
	private boolean inheritProperty(@NotNull ControlPropertyLookupConstant lookup, @NotNull ControlClassOld extendClass) {
	...
	/**
	 In order for a property to be overridden, the property exists in this {@link ControlClassOld},
	...
	public final Iterable<ControlProperty> getAllChildProperties() {
		ArrayList<List<ControlProperty>> merge = new ArrayList<>(2);
	...
	/**
	 ...
	 @return all {@link ControlClassOld} instances that exist only because of inheritance.
	 */
	@NotNull
	public final ReadOnlyList<ControlClassOld> getTempNestedClassesReadOnly() {
	...
	/**
	 ...
	 across the program, the ControlClassOld listens to it's own ControlProperties.
	 ...
	 If this ControlClassOld extends some ControlClassOld via
	 {@link #extendControlClass(ControlClassOld)}, the update groups will <b>not</b> be synced.
	 You will have to listen to each ControlClassOld separately.
	 */
	@NotNull
	...
	/**
	 Gets the update listener group that listens to when an update happens to this {@link ControlClassOld}.
	 ...
	 <li>{@link #extendControlClass(ControlClassOld)}</li>
	...
	/**
	 Checks if the given {@link ControlClassOld} matches the following criteria:<br>
	 <ul>
	 ...
	 */
	public boolean classEquals(@NotNull ControlClassOld controlClass) {
		...
		for (ControlClassOld myNested : getAllNestedClasses()) {
			ControlClassOld other = controlClass.findNestedClassNullable(myNested.getClassName());
			if (other == null) {
				return false;
			}
	...
	/**
	 Sets this {@link ControlClassOld} fully equal to <code>controlClass</code> (disregards {@link #getUserData()}).
	 NOTE: nothing will be deep copied!

	 @param controlClass class to use
	 */
	public final void setTo(@NotNull ControlClassOld controlClass) {
		...
		for (ControlClassOld nested : controlClass.getAllNestedClasses()) {
			ControlClassOld m = findNestedClassNullable(nested.getClassName());
			if (m == null) {
				optionalNestedClasses.add(nested);
			} else {
				m.setTo(nested);
	...
	/** Will set all {@link ControlClassOld} instances that have this as their {@link #getExtendClass()} and set it to null */
	public void clearSubClasses() {
		List<ControlClassOld> unextend = new ArrayList<>(mySubClasses.size());
		unextend.addAll(mySubClasses);
		for (ControlClassOld cc : unextend) {
			cc.extendControlClass(null); //todo this doesn't work
		}
	}

	/**
	 Updates this {@link ControlClassOld} to the given update
	...
	public final void update(@NotNull ControlClassUpdate data, boolean deepCopy) {
		if (data instanceof ControlClassRenameUpdate) {
			...
			ControlClassOld nestedClassToUpdate;
			ControlClassOld myNestedClass;
			ControlClassOld parentClass = this;
			do {
