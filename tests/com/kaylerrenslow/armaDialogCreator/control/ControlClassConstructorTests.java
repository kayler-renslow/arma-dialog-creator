...
/**
 @author Kayler
 @since 05/15/2017 */
public class ControlClassConstructorTests {
	@Test
	public void controlClassConstructor() throws Exception {
		...
		if (!c.getRequiredNestedClasses().get(0).classEquals(reqNested1.constructNewControlClass(specRegistry))) {
			assertEquals("Required nested class 1 of the constructed control class isn't equal to expected ControlClass", true, false);
		}

		if (!c.getRequiredNestedClasses().get(1).classEquals(reqNested2.constructNewControlClass(specRegistry))) {
			assertEquals("Required nested class 2 of the constructed control class isn't equal to expected ControlClass", true, false);
		}

		if (!c.getOptionalNestedClasses().get(0).classEquals(optNested1.constructNewControlClass(specRegistry))) {
			assertEquals("Optional nested class 1 of the constructed control class isn't equal to expected ControlClass", true, false);
		}

		if (!c.getOptionalNestedClasses().get(1).classEquals(optNested2.constructNewControlClass(specRegistry))) {
			assertEquals("Optional nested class 2 of the constructed control class isn't equal to expected ControlClass", true, false);
		}
		testPropertiesEqual(requiredProperties, optionalProperties, c);
	}

	/**
	 helper method for seeing if a {@link ControlClass} has the required and optional properties given.
	 This method will report any missing properties through the testing library.
	 Note: This will not mutate the given arrays!
	 */
	private void testPropertiesEqual(ControlPropertyLookupConstant[] requiredProperties, ControlPropertyLookupConstant[] optionalProperties, ControlClass c) {
